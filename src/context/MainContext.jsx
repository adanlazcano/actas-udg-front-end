...
const MainProvider = ({ children }) => {
  ...
  const [loading, setLoading] = useState("Cargando Información...");
  ...
  const searchStudent = (search) => {
    search.value.length > 0 ? getSearch(search.value) : getData();
    setLoading("No se han encontrado resultados");
  };
  ...
  const actaPdf = (file, title, date = 0) => {
    ...
    element
      .querySelectorAll(".printActa")
      .forEach((item) => (item.innerHTML = title));
    element
      .querySelectorAll(".printDate")
      .forEach(
        (item) =>
          (item.innerHTML = new Intl.DateTimeFormat("es-ES", {
            dateStyle: "full",
          }).format(date))
      );
